...
for line in input_file:
    count+=1
    if count>5  and count < 10 and line.split()[0] == str(args.n):
        ref_cons = line.split()[1]
...
for line in input_file6:
    count+=1
    if count>5  and count < 10 and line.split()[0] == str(args.n):
        ref_pareto=line.split()[1]
count=0
for line in input_file7:
    count+=1
    if count>5  and count < 10 and line.split()[0] == str(args.n):
        ref_parallel=line.split()[1]

roc=str(args.set) + "," +str(ref1) + "," +  str(ref2) + "," + str(ref3) + "," + str(ref4) + "," + str(ref5) + "," + str(ref_cons)+ "," + str(ref_pareto) +","+ str(ref_parallel) +"\n"
